...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-3">
        <div class="col-sm-7">
          <h3 style="color: white;">User Pages</h3>
        </div>
      </div>
    </div>
  </section>
      ...
      <div class="col-12">
        <div class="card mb-4">
          <div class="card-header pb-0 d-flex justify-content-between align-items-center">
            <h6 class="text-uppercase  text-sm font-weight-bolder" style="color: rgb(56, 56, 56)">User SMKN 1 SUBANG</h6>
            <a href="{{ route('user.create') }}" class="btn btn-outline-primary btn-sm mb-0">Tambah User</a>
              ...
              <table id="myTable" class="display">
                <thead>
                    <tr>
                      ...
                      <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">Username</th>
                  ...
                  <tbody>
                    ...
                    @foreach ($userM as $siswa)
                    <tr>
                      <td class=" text-secondary text-sm font-weight-bolder opacity-14">{{ $siswa->name }}</td>
                      <td class=" text-secondary text-sm font-weight-bolder opacity-14">{{ $siswa->username }}</td>
                      <td class=" text-secondary text-sm font-weight-bolder opacity-14">{{ $siswa->role }}</td>
                
                      <td>
                        <form action="{{ route('user.destroy', $siswa->id) }}" method="POST">
                            <a href="{{ route('user.edit', $siswa->id) }}" class="btn btn-outline-success btn-xm mb-0">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="{{ url('user/changepassword', $siswa->id)}}" class="btn btn-outline-info btn-xm mb-0">
